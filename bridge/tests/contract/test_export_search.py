...
def test_search_exports_basic(contract_client: TestClient) -> None:
    """Verify the exports search endpoint returns the expected envelope."""
    ...
    assert data["query"] == "export"
    assert isinstance(data["total_results"], int)
    assert data["page"] == 0
    assert data["limit"] == 10
    assert isinstance(data["items"], list)
    assert data["items"], "Expected at least one export match"
...
def test_search_exports_validates_schema(contract_client: TestClient) -> None:
