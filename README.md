...
>
> **Status:** Experimental • **License:** Apache 2.0
...
* **Contextual search**
  `search_scalars_with_context` returns matches plus a server-side disassembly window around them, so clients don't need extra “give me the surrounding instructions” calls.
...
* **Search & analysis** – `search_*`, `string_xrefs`, `list_functions_in_range`, etc.
* **Memory & disassembly** – `read_*`, `disassemble_*`
* **Advanced analysis** – `analyze_function_complete`, jump-table helpers
* **Data & utilities** – `strings_compact`, `project_*`, `write_bytes`, `health`
...
* [Server operations](docs/server.md) – Running and managing the server (SSE vs stdio, `/state`, readiness, etc.)
* [API reference](docs/api.md) – Endpoints, payloads, and examples
